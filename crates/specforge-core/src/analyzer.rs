...
pub fn analyze_spec(doc: &Document) -> AnalysisReport {
    ...
    if let Ok(json) = serde_json::to_vec(&doc) {
        report.total_size_bytes = json.len();
    }
    let mut referenced: HashSet<String> = HashSet::new();
    for op in &doc.operations {
        if let Some(body) = &op.request_body {
            collect_refs_from_type(&body.ty, &mut referenced);
        }
        for resp in &op.responses {
            if let Some(ref body) = resp.body {
                collect_refs_from_type(body, &mut referenced);
            }
        }
    }
    for wh in &doc.webhooks {
        if let Some(body) = &wh.request_body {
            collect_refs_from_type(&body.ty, &mut referenced);
        }
        for resp in &wh.responses {
            if let Some(ref body) = resp.body {
                collect_refs_from_type(body, &mut referenced);
            }
        }
    ...
    for name in doc.schemas.models.keys() {
        if !referenced.contains(name) {
            report.unused_schemas.push(name.clone());
        }
    }
    ...
    for (name, model) in doc.schemas.iter() {
        if let Model::Object(obj) = model {
            if obj.properties.len() > 20 {
                report
                    .large_schemas
                    .push((name.clone(), obj.properties.len()));
            }
        }
    }
        ...
        if group.len() > 1 {
            for i in 0..group.len() {
                for j in (i + 1)..group.len() {
                    dupes.push((group[i].clone(), group[j].clone()));
                }
            }
        }
    }
    ...
    for name in doc.schemas.models.keys() {
        ...
        if depth > 2 {
            report.deep_refs.push((name.clone(), depth));
        }
    }
    report.deep_refs.sort_by_key(|a| Reverse(a.1));
    if !report.unused_schemas.is_empty() {
        report.recommendations.push(format!(
            "Remove {} unused schema(s): {}",
            report.unused_schemas.len(),
            report.unused_schemas.join(", ")
        ));
    }
    if !report.duplicate_schemas.is_empty() {
        report.recommendations.push(format!(
            "Found {} pair(s) of schemas with identical property sets.",
            report.duplicate_schemas.len()
        ));
    }
    for (name, props) in &report.large_schemas {
        report.recommendations.push(format!(
            "Schema '{}' has {} properties. Consider splitting.",
            name, props
        ));
    }
    for (name, depth) in &report.deep_refs {
        report.recommendations.push(format!(
            "Schema '{}' has deep reference chain (depth {}).",
            name, depth
        ));
    }
    report
...
fn collect_refs_from_type(ty: &Type, refs: &mut HashSet<String>) {
    match ty {
        ...
        Type::Array { item, .. } => {
            collect_refs_from_type(item, refs);
        }
        Type::Map { value } => {
            collect_refs_from_type(value, refs);
        }
        Type::Composition(comp) => {
            for m in &comp.members {
                collect_refs_from_type(m, refs);
            }
        }
        _ => {}
    }
    ...
    match model {
        Model::Object(obj) => {
            for prop in &obj.properties {
                collect_refs_from_type(&prop.ty, refs);
            }
            if let Some(ref ap) = obj.additional_properties {
                collect_refs_from_type(ap, refs);
            }
            if let Some(ref shape) = obj.shape_type {
                collect_refs_from_type(shape, refs);
            }
            if let Some(ref base) = obj.base_type {
                collect_refs_from_type(base, refs);
            }
        }
        Model::Enum(_) => {}
    }

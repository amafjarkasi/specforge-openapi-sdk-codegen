        ...
        match model {
            Model::Object(obj) => {
                ...
                let label = obj
                    .description
                    .as_deref()
                    .map_or_else(|| name.clone(), |d| format!("{name}\n{d}"));
                g.nodes.push(Node {
                    id: name.clone(),
                    label,
                    shape,
                });
                if let Some(ref ty) = obj.shape_type {
                    for d in collect_references(ty) {
                        add_edge(&mut g.edges, name, &d, None);
                    }
                }
                if let Some(ref base) = obj.base_type {
                    for d in collect_references(base) {
                        add_edge(&mut g.edges, name, &d, Some("extends"));
                    }
                }
                for prop in &obj.properties {
                    for d in collect_references(&prop.ty) {
                        add_edge(&mut g.edges, name, &d, None);
                    }
                }
                if let Some(ref ap) = obj.additional_properties {
                    for d in collect_references(ap) {
                        add_edge(&mut g.edges, name, &d, Some("values"));
                    }
                }
            }
            Model::Enum(e) => {
...
fn mlbl(l: &str) -> String {
    l.replace('\\', "\\\\").replace('"', "#quot;")
}
fn did(name: &str) -> String {
    format!("\"{}\"", name.replace('"', "\\\""))
}
fn dlbl(l: &str) -> String {
    l.replace('\\', "\\\\")
        .replace('"', "\\\"")
        .replace('\n', "\\l")
}

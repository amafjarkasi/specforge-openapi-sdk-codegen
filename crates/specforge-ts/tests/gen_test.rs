...
fn output_is_deterministic() {
    ...
    let spec = specforge_core::parse_file(&spec_path).expect("parse");
    let doc = specforge_core::resolve(&spec).expect("resolve");
    let opts = |dir: &std::path::Path| specforge_ts::GeneratorOptions {
    ...
    let files_a = specforge_ts::generate(&doc, &opts(dir_a.path())).expect("emit A");
    let files_b = specforge_ts::generate(&doc, &opts(dir_b.path())).expect("emit B");
